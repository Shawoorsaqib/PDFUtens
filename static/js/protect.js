document.addEventListener("DOMContentLoaded", () => {
    const uploadForm = document.getElementById("protectPdfForm");
    const uploadContent = document.getElementById("uploadContent");
    const chooseBtn = document.getElementById("chooseProtectBtn");
    const fileInput = document.getElementById("protectPdfFileInput");
    const selectedFilesContainer = document.getElementById("selectedFilesContainer");
    const uploadActions = document.getElementById("uploadActions");
    const uploadBtn = document.getElementById("protectPdfSubmitBtn");
    const uploadStatus = document.getElementById("uploadStatus");
    const successCard = document.getElementById("successCard");
    const downloadBtn = document.getElementById("downloadProtectedBtn");
    ...
    uploadForm.addEventListener("submit", async e => {
        ...
        if (!pdfPasswordInput || !pdfPasswordInput.value.trim()) {
            if (uploadStatus) {
                uploadStatus.style.display = "block";
                uploadStatus.className = "upload-status upload-error";
                uploadStatus.textContent = "⚠ Please enter a password to protect your PDF.";
            }
            return;
        }
        ...
        if (uploadBtn) {
            ...
            uploadBtn.innerHTML = `<i class="bi bi-arrow-repeat spin"></i> Encrypting PDF...`;
        }

        if (uploadStatus) {
            uploadStatus.style.display = "block";
            uploadStatus.className = "upload-status upload-loading";
            uploadStatus.textContent = "Protecting PDF file, please wait...";
        }

        const formData = new FormData();
        formData.append("file", selectedFile);
        formData.append("password", pdfPasswordInput.value.trim());

        try {
            const response = await fetch("/protect-pdf/upload", {
                method: "POST",
                body: formData
            });
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || "Failed to protect PDF.");
            }
            ...
            if (downloadBtn) {
                ...
                downloadBtn.setAttribute("download", data.filename || "protected.pdf");
                downloadBtn.innerHTML = `<i class="bi bi-download"></i> Download Protected PDF`;
            }
            ...
            if (uploadBtn) {
                ...
                uploadBtn.innerHTML = `<i class="bi bi-shield-lock-fill"></i> Protect PDF File`;
            }
        }
    });
