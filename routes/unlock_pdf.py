...
from utils.pdf_utils import unlock_pdf_file
...
@unlock_pdf_bp.route("/unlock-pdf")
def unlock_pdf():
    """Renders Unlock PDF tool page."""
    return render_template("tools/unlock_pdf.html")


@unlock_pdf_bp.route("/unlock-pdf/upload", methods=["POST"])
def upload_unlock_pdf():
    """
    Decrypts encrypted PDF using provided password and returns download payload.
    """
    cleanup_old_files()
    ...
    try:
        output_filename, output_path = unlock_pdf_file(uploaded_path, password=password)
        ...
        return jsonify({
            ...
            "download_url": f"/unlock-pdf/download/{output_filename}"
        })
...
@unlock_pdf_bp.route("/unlock-pdf/download/<path:filename>")
@unlock_pdf_bp.route("/unlock-pdf/download/<filename>")
def download_unlocked_pdf(filename):
    """
    Serves decrypted PDF and cleans up file from disk.
    """
